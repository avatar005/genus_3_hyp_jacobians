...
def write_vs(n):
    ...
    for poly in out:
        with open(os.path.join("data"+str(2**n), "poly_" + str(poly)), "w+") as file:
            file.write(str(poly.list()))

    return(len(out))


def generate_hyperelliptics_char2(n, v):
    """
    ...
    # interpret v as a polynomial
    v = sage_eval(v, locals={'a':a})
    v = R(v)
