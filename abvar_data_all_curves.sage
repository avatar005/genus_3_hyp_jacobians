...
def get_data_initial_all_curve(prefetch=None):
    '''Downloads data from database. Pickles resulting fetched data as a dictionary keyed by hyperelliptic classification; (factoring, slope type), field. 
    Each entry contains polynomial, coefficients in factored form, and LMFDB label'''
    ...
    for q in (2, 3, 5):
        ...
        # fetch data
        for rec in db.av_fq_isog.search(fetch_keys, ["label", "q", "jacobian_count", "poly", "slopes", "simple_factors"]):
            q = rec["q"]
            slope = slope_types[tuple(rec["slopes"])]
            have_jac = (rec["jacobian_count"] > 0)
            factors = rec["simple_factors"]
            for i in range(len(factors)):
                factors[i] = prefetch[factors[i][:-1]]
            factoring_type = tuple([len(factors[i]) - 1 for i in range(len(factors))])
            coeffs = get_abc(factors, factoring_type)
            data[have_jac][factoring_type, slope][q].append((rec["poly"], coeffs, rec["label"]))
...
def pickle_data(data):
    ...
    with open("data_all.dict", 'wb') as filename:
...
def write_data(data, excluded={}):
    ...
    for dir in os.listdir('new'):
        try:
            shutil.rmtree(os.path.join('new', dir))
    ...
    for have_hyp, D in data.items():
        ...
        for (factoring, slopes), L2 in D.items():
            dir = os.path.join('new', f'{factoring}_{slopes}')
...
def plot(data, factoring, slope_type, q, factored=True):
    ...
    q == 1: odd
    q == -1: squares
    Plots everything in a single figure with multiple 3d plots. Allows toggles for the two datasets/ Parameter "factored" controls whether the factored
    or unfactored coefficients are factored'''
    def restriction(x, y, z, q):
        ...
        zn = z_mod(x, y, z, q)
        # return True
        return round(abs(zn)) == (yn + q)/q**0.5 and zn >= 0
    def x_mod(x, y, z, q):
        '''Allows one to apply a modification to the x variable. '''
        return (x - 2*q*z)/((2*q)**2 + 1)**0.5
    def y_mod(x, y, z, q):
    ...
    sc2 = []
    if q == 0:
        pr = [2, 4, 8, 16], plt.figure(num=1), 2, 2
    elif q == 1:
...
def sort_data_all_curves(data=None, source=False, dest='undetected'):
...
def mod_2_count(data):
    ...
    counts_f = 0
    mod = 2
    for exp in ((2,2,2), (2, 4), (6,)):
        for slope in [0, 1, 2, 3, 4]:

            for hyp in [True, False]:
                for poly in data[hyp][exp, slope][q]:
                    for i in range(mod):
                        for j in range(mod):
                            for k in range(mod):
                                if tuple([i%mod for i in poly[0][1:4]]) == (i, j, k):
                                    counts[i, j, k] += 1
                                    if hyp == True:
                                        counts_t += 1
                                    else:
...
def n_tuple(length, lower, upper):
    '''Returns an iterable of length entries each ranging from lower to upper, inclusive'''
...
class obsolete:
    def write_data_with_point_counts(data):
        '''OBSOLETE. Write data with stored point counts.'''
        for have_hyp, D in data.items():
            for (factoring, slopes), L2 in D.items():
                for q, polys in L2.items():
                    ...
                    with open(filename, "w") as F:
                        for poly in polys:
                            if factoring == (6,):
                                ...
                                F.write(f"{poly[1]}, {poly[0]}, {poly[2]}, {gcd(quadratic_point_count(poly[1][0],q), quadratic_point_count(poly[1][1],q))} \n")
                            else:
                                point_counts = [quadratic_point_count(m, q) for m in poly[1]]
                                F.write(f"{poly[1]}, {poly[0]}, {poly[2]}, {point_counts}, {gcd(point_counts[0], point_counts[1]), gcd(point_counts[1], point_counts[2]), gcd(point_counts[0], point_counts[2])} \n")

    def quadratic_point_count(mid_coeff, q):
        '''Helper function for writing data with point counts'''
        return 1 + mid_coeff + q

    def all_coeff_patterns(mod):
        '''generates all patterns modulo "mod"'''
        for i in range(mod):
            for j in range(mod):
                for k in range(mod):
                    yield (i, j, k)
    ...
    def mod_2_checks(data):
        '''Compute which mod 2 patterns hold for the coefficients of the polynomials.'''
    ...
    def poly_rule_check(data):
        '''Check relevance and truthfullness of all rules of the form ax^2 + by^2 + cz^2 + dx + ey + fz for coefficients x, y, z. 
        Only considers patterns with more than 4 hits per field.'''
        def evaluate(coeffs, input):
            '''evaluates the polynomial'''
            dic = {0:0, 1:1, 2:-1, 3:4, 4:-4}
            a, b, c, d, e, f = [dic[i] for i in coeffs]
            x1, x2, x3 = input
